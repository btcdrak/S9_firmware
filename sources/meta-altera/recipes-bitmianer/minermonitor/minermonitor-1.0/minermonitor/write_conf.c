...
char *read_s4_switch(char* fileName,char *key,char *value)
{
    ...
    cJSON *jsonroot = cJSON_Parse(pContent); //json栿
...
POOLS_D_COUNT *rev_pools(char *buf_tmp ,POOLS_D_COUNT *get_pool_data)
{
  ...
  //buf_tmp=pkg_struct.pkg_body;
    ...
    *(body_tmp++) = '\0';
    strcpy(rev_ID,buf_tmp);  //获取一丿ID
    buf_tmp=body_tmp;

	while (buf_tmp != NULL) {
		nextobj = strchr(buf_tmp, ';'); //;符号切断
		if (nextobj != NULL)
			*(nextobj++) = '\0';
		if (*buf_tmp) {
			item = buf_tmp;
...
GETS4_CONFIG *save_old_conf(char* fileName,GETS4_CONFIG *get_save_conf)
{
    ...
    cJSON *jsonroot = cJSON_Parse(pContent); //解析
    cJSON *taskArry=cJSON_GetObjectItem(jsonroot,"pools");//取数组
    //int arrySize=
    cJSON_GetArraySize(taskArry);//数组大小
    cJSON *tasklist=taskArry->child;//子项目个数
...
if(change->pools_count==1) //有一个矿池改bian
{
    int which_p;
    which_p=atoi(change->pools_data[0].which_pool);//哪一个pool change
    switch(which_p)
    {
        case 0:
...
else if(change->pools_count==2)
{
    ...
    which_p1=atoi(change->pools_data[0].which_pool);//哪一个改bian
    which_p2=atoi(change->pools_data[1].which_pool);//哪一个改bian
    switch(which_p1)
    {
        case 0:
        strcpy(fresh_old->pools_data[0].url ,change->pools_data[0].url);
...
GETS3_CONFIG *creat_s3_newpools(POOLS_D_COUNT *s3_change,GETS3_CONFIG *refresh_s3)
{
  ...
  { which_p1=atoi(s3_change->pools_data[0].which_pool);//哪一个改bian
  ...
  { which_p1=atoi(s3_change->pools_data[0].which_pool);//哪一个改bian
    which_p2=atoi(s3_change->pools_data[1].which_pool);//哪一个改bian
    strcpy(refresh_s3->pools_data[which_p1].url ,s3_change->pools_data[0].url);
    strcpy(refresh_s3->pools_data[which_p1].user ,s3_change->pools_data[0].user);
    strcpy(refresh_s3->pools_data[which_p1].pass ,s3_change->pools_data[0].pass);
